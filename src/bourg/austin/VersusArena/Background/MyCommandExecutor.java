	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
	{
		...
		if (cmd.getName().equalsIgnoreCase("versus"))
		{
			...
			else if (args.length >= 2)
			{
				...
				{
					//If setdeath is being used
					if (args[1].equalsIgnoreCase("setdeath") && sender.hasPermission("pairspvp.arena.setdeath"))
					{
						...
						if (player != null)
						{
							...
							{
								//If arena exists
								Arena tempArena = plugin.getArenaManager().getArena(args[2]);
								if (tempArena != null)
								{
									//If the player has a location selected
									Location tempLoc = plugin.getSelectedLocation(player.getName());
									if (tempLoc != null)
									{
										tempArena.setDeathLocation(tempLoc);
										player.sendMessage(ChatColor.GREEN + "The death spawn has been set at the selected location.");
									}
									//If the player has no location selected
									else
										sender.sendMessage(ChatColor.RED + "You must select a location for the death spawn");
								}
								//If the arena doesn't exist
								else
									sender.sendMessage(ChatColor.RED + "There is no arena named " + args[2]);
							}
							//If the wrong number of args is supplied
							else
								sender.sendMessage(ChatColor.RED + "/versus arena setdeath <name>");
						}
						//If the sender is the console
						else
							sender.sendMessage(ChatColor.RED + "Only players can use /versus arena setdeath <name>");
					}
					
					//If the user is trying to make a new arena
					else if (args[1].equalsIgnoreCase("make") && sender.hasPermission("pairspvp.arena.make"))
					{
					...
					else if (args[1].equalsIgnoreCase("details") && sender.hasPermission("pairspvp.arena.details"))
					{
						if (args.length == 3)
						{
							...
							if (plugin.getArenaManager().containsArena(args[2]))
							{
								...
								tempLoc = plugin.getArenaManager().getArena(args[2]).getDeathLocation();
								sender.sendMessage(ChatColor.DARK_AQUA + "Death Spawn: " + (tempLoc == null ? ChatColor.RED + "Not set" : ChatColor.GREEN +   "(" + tempLoc.getBlockX() + ", " + tempLoc.getBlockY() + ", " + tempLoc.getBlockZ() + ") in world " + tempLoc.getWorld().getName()));															
					...
					else if (args[1].equalsIgnoreCase("setfacing") && player.hasPermission("pairspvp.arena.setfacing"))
					{
						//If the command sender is a player
						if (sender instanceof Player)
						{
							...
							if (plugin.getArenaManager().containsArena(args[2]))
							{
								...
								//If the wrong number of arguments has been supplied
									...
									sender.sendMessage(ChatColor.RED + "/versus arena setfacing <arenaname> <teamnum> <playernum> <optional direction>");								
							}
							//If the arena doesn't exist
							else
								sender.sendMessage(ChatColor.RED + "The arena " + ChatColor.BLUE + args[2] + ChatColor.RED + " does not exist");
					...
					else if (args[1].equalsIgnoreCase("setspawn") && player.hasPermission("pairspvp.arena.setspawn"))
					{
						//If the command sender is a player
						if (sender instanceof Player)
						{
